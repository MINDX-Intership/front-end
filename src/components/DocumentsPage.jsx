...
const DocumentsPage = ({ authToken, setCurrentPage, currentUser }) => {
    ...
    useEffect(() => {
        if (!loading && !error) { // Only start delay timer when actual loading is complete and no error
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:3000/api/documents/${documentToDelete._id}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${authToken}`,
    ...
    const handleSaveDocument = async () => {
        ...
        try {
            ...
            if (response.ok) {
                toast.success(`Tài liệu đã được ${currentDocument._id ? 'cập nhật' : 'tạo'} thành công!`);
                fetchDocuments(); // Refresh the list
                handleCloseEditDialog();
            } else {
                toast.error(data.message || `Lỗi khi ${currentDocument._id ? 'cập nhật' : 'tạo'} tài liệu.`);
            }
        } catch (err) {
        ...
        return (
            <LoadingContainer>
                <LoadingContent>
                    ...
                    <CreateButton onClick={fetchDocuments}>
                        Thử lại
                    </CreateButton>
                </LoadingContent>
            </LoadingContainer>
